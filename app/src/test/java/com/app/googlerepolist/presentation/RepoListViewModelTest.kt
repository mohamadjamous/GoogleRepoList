...
class RepoListViewModelTest {
    ...
    fun setup() {
        val fakeRepo = object : RepoRepository {
            override suspend fun getRepos() = listOf(
                RepoDto(
                    id = 1,
                    name = "TestRepo",
            ...
            override suspend fun getRepoBydId(repoId: String) = TODO()
        }
        viewModel = RepoListViewModel(GetReposUseCase(fakeRepo))
    }

    @Test
    fun `repos are loaded correctly`() = runTest {
        advanceUntilIdle()
        assert(viewModel.state.value.repos.isNotEmpty())
    }

    @Test
    fun `searchQuery filters repos`() = runTest {
        viewModel.onSearchQueryChanged("Jet")
        assertTrue(viewModel.filteredRepos.all { it.name.contains("Jet") })
    }
}
